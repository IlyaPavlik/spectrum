...
internal class ColorCutQuantizer(
    ...
    private var _quantizedColors: MutableList<Swatch>? = null
    private val _tempHsl: FloatArray = FloatArray(3)
    private val _histogram: IntArray = IntArray(1 shl (QUANTIZE_WORD_WIDTH * 3))

    init {
        val hist: IntArray = _histogram
        for (i in pixels.indices) {
            val quantizedColor: Int = quantizeFromRgb888(pixels[i])
            // Now update the pixel value to the quantized value
            pixels[i] = quantizedColor
            // And update the histogram
            hist[quantizedColor] = hist[quantizedColor].inc()
        }
        ...
        _colors = IntArray(distinctColorCount)
        val colors: IntArray = _colors
        ...
        _quantizedColors = if (distinctColorCount <= maxColors) {
            // The image has fewer colors than the maximum requested, so just return the colors
            ArrayList<Swatch>().apply {
                for (color: Int in colors) {
                    add(Swatch(approximateToRgb888(color), hist[color]))
                }
            }
        } else {
    ...
    private fun quantizePixels(maxColors: Int): MutableList<Swatch> {
        ...
        pq.offer(Vbox(0, _colors.size - 1))
    ...
    private fun shouldIgnoreColor(color565: Int): Boolean {
        ...
        ColorUtils.colorToHSL(rgb, _tempHsl)
    ...
    companion object {
        ...
        private const val COMPONENT_RED: Int = -3
        private const val COMPONENT_GREEN: Int = -2
        private const val COMPONENT_BLUE: Int = -1
